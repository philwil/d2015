...
#include <../OWNCODE/DPS_general.h>	// new since 27 oct 2010. /* modified jan 5th 2011 */
...
__attribute__((always_inline))
static inline void serial_early_puts2(const char *s)
{
	serial_puts("Early2: ");
	serial_puts(s);
}
...
void init_cplbtables(void)
{
	...
	icplb_add(0xFFA00000, L1_IMEMORY | CPLB_LOCK );
	dcplb_add(0xFF800000, L1_DMEMORY | CPLB_LOCK);
	++i;
	...
	if (CONFIG_MEM_SIZE) {
		...
		mend &= CPLB_PAGE_MASK;
		icplb_add(mbase, SDRAM_IKERNEL | CPLB_LOCK);
		dcplb_add(mbase, SDRAM_DKERNEL | CPLB_LOCK);
		++i;
	...
	icplb_add(0x20000000, SDRAM_INON_CHBL | CPLB_LOCK);
	dcplb_add(0x20000000, SDRAM_EBIU | CPLB_LOCK );
	++i;
...
//#ifdef CONFIG_DEBUG_NULL_PTR
	...
	++i;
	icplb_add(extern_memory, SDRAM_IKERNEL | CPLB_LOCK);
	dcplb_add(extern_memory, SDRAM_DKERNEL | CPLB_LOCK);
	extern_memory += CPLB_PAGE_SIZE;
	++i;
//#endif

	while (i < 16 && extern_memory < (CONFIG_SYS_MONITOR_BASE & CPLB_PAGE_MASK)) {
		icplb_add(extern_memory, SDRAM_IGENERIC | CPLB_LOCK );
		dcplb_add(extern_memory, SDRAM_DGENERIC | CPLB_LOCK);
		extern_memory += CPLB_PAGE_SIZE;
		++i;
	}
//	icplb_add(0xEF000000, L1_IMEMORY | CPLB_LOCK );
//	dcplb_add(0, 0);
//	++i;
...
//	puts("KGDB:  ");
	kgdb_init();
#endif
 //      printf(" PSW note setting ipaddr in board.c \n");
//       setenv("ipaddr","10.1.0.82");  
...
//eth_init(bd);// does not setup the ethernet as Phil expected.

	Own_Code();		// /* modified jan 5th 2011 */
