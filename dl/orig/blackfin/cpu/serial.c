...
extern int run_owncode;
extern int nmi_poll;
extern int Springboard_Own_Code(void); //NULL please// own code
extern int Do_Own_Code_DE(void);			// own code. Located in own_process_manager
int serial_getc(void)
{
	...
	while (!serial_tstc()) {
	  if(nmi_poll) {
	    if(run_owncode) {
	      run_owncode = 0;
	      Do_Own_Code_DE(); //NULL please// own code
	    }
	  }
	}
	/* grab the new byte */
...
extern int ok_printf;			// u-boot run command !!!
void serial_puts(const char *s)
{
     if(ok_printf==0) return;		// u-boot run command !!!
	while (*s)
		serial_putc(*s++);
}
