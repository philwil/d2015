...
int trap_c(struct pt_regs *regs, uint32_t level)
{
	...
	uint32_t raw_trapnr = regs->seqstat;
     char str_dat[64];
	unsigned long tflags;
	bool data = false;

	hwerrcause = (regs->seqstat & HWERRCAUSE) >> HWERRCAUSE_P;
	excause = (regs->seqstat & EXCAUSE) >> EXCAUSE_P;

     puts("\n\n trap \n");
     sprintf(str_dat,"*** seq %08x ",raw_trapnr);
     puts(str_dat);

     sprintf(str_dat,"| retx %08x ",(unsigned int)regs->retx);
     puts(str_dat);

     iaddr = bfin_read_ICPLB_FAULT_ADDR();
     sprintf(str_dat,"| iaddr %08x ",(unsigned int)iaddr);
     puts(str_dat);

     daddr = bfin_read_DCPLB_FAULT_ADDR();
     sprintf(str_dat,"| daddr %08x \n",(unsigned int)daddr);
     puts(str_dat);

//     puts(strexcause((uint16_t) excause));
//     puts("***\n\n hw:");
//     puts(strhwerrcause((uint16_t) hwerrcause));
//     puts("***\n\n");
	...
	/* 0x27 - Multiple Data CPLB Hit  */
        /* SDCS lets ignore this one */
//	case VEC_CPLB_MHIT:
//	  break;
	...
	case VEC_CPLB_M:
     ...
     /* SDCS This is really 0x2c and we should ignore it like 0x26 */
...
# error cplb enable bit violates my sanity
	...
	default:
		...
		if (debugger_exception_handler && (*debugger_exception_handler)(regs))
			break;
#endif
		bfin_panic_str(regs, " sdcs trap c");
	}
...
void bfin_dump(struct pt_regs *regs)
{
	...
	puts(
		...
		"\n"
		"\n"
		"Ack! Something bad happened to the Blackfin! (sdcs)n"
		"\n"
	);
	dump(regs);
	_dump_bfin_trace_buffer();
...
void bfin_panic_irq(struct pt_regs *regs)
{
    bfin_panic_str(regs, "Irq" );
}
...
void bfin_panic_str(struct pt_regs *regs, char *str)
{
	unsigned long tflags;
     puts(str);
	trace_buffer_save(tflags);
