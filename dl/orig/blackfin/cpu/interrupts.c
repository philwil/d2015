...
void __udelay(unsigned long usec)
{
	...
	unsigned long cclk;
     ...
     //udelay_de(usec);	// this made everything working!!  this function is not tuned and not reproducable. located in time.c (/lib)
     usdelay(usec); // in owncode.c this one is calibrated	11 may 2011
     return;
}
#if 0
	cclk = (CONFIG_CCLK_HZ);
	...
	while (usec > 1) {
		...
		asm volatile (" %0 = CYCLESnotused;" : "=r" (start));
		do {
			asm volatile (" %0 = CYCLESnotused; " : "=r" (stop));
 ...
 * sufficient
 */
ulong get_timer0(ulong base)
{
	ulong milisec;

	/* Number of clocks elapsed */
//	ulong clocks = (MAX_TIM_LOAD - bfin_read_TCOUNT());
//        ulong clocks = ((*pTPERIOD) - (*pTCOUNT));
	ulong clocks = ( bfin_read_TPERIOD() - bfin_read_TCOUNT() ) ;
	...
	//milisec += timestamp * (MAX_TIM_LOAD / (CONFIG_CCLK_HZ / 1000));
        milisec += timestamp * (bfin_read_TPERIOD() /\
                                 (CONFIG_CCLK_HZ / 1000));
...
{
	if(use_tmr3) return get_timer3(base);
	return get_timer0(base);
}
